  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-[var(--primary-color)]">Sessions</h3>
        {canManage && !showForm && (
      ...
      {showForm && (
        <div className="flex flex-col gap-2 border border-[var(--primary-gray)] rounded-[10px] p-4">
          <label className="text-sm text-[var(--member-color)]">
      ...
      {sessions.length === 0 && !showForm && (
        <p className="text-sm text-[var(--member-color)]">No sessions yet.</p>
      )}
      ...
      <ul className="divide-y divide-[var(--primary-gray)]">
        {sessions.map((s) => (
          ...
          >
            <div>
              <span className={`text-sm font-semibold capitalize ${STATUS_COLORS[s.status]}`}>
                {s.status}
              </span>
              {s.scheduled_for && (
                <span className="text-xs text-[var(--member-color)] ml-3">
                  {new Date(s.scheduled_for).toLocaleString()}
                </span>
              )}
            </div>
            <span className="text-xs text-[var(--member-color)]">
